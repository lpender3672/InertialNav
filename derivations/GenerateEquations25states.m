...
% sensor assumed to be aligned with the Z body axis plus a small
% misalignment
% 25 state architecture (increased from 22 to include rate random walk states).
...
% Modified for balanced IMU error model implementation
...
% Delta Angle rate random walk - rad (X,Y,Z)  % NEW: Added for balanced implementation
...
syms dax_rw day_rw daz_rw real % delta angle rate random walk states - rad % NEW
...
syms dax_rwCov day_rwCov daz_rwCov real; % rate random walk process noise - rad^2 % NEW
...
syms SF SG SQ SPP SK_VP SH_TAS SK_TAS SH_MAG SK_MX SK_MY SK_MZ SH_LOS SK_LOS SH_RNG SK_RNG SH_ACCX SK_ACCX SH_ACCY SK_ACCY SH_MAGS SK_MAGS real;

%% define the process equations

% Define the state vector & number of states
% Modified to include rate random walk states after gyro bias states
stateVector = [q0;q1;q2;q3;vn;ve;vd;pn;pe;pd;dax_b;day_b;daz_b;dax_rw;day_rw;daz_rw;dvz_b;vwn;vwe;magN;magE;magD;magX;magY;magZ];
...
% Modified to include both bias and rate random walk contributions
da_b = [dax_b; day_b; daz_b];
da_rw = [dax_rw; day_rw; daz_rw];
dv_b = [0; 0; dvz_b];
...
% Modified to subtract both bias and rate random walk errors
dAngCor = da - da_b - da_rw;
dVelCor = dv - dv_b;
...
% Gyro bias states remain constant (Gauss-Markov or constant bias model)
dabNew = [dax_b; day_b; daz_b];

% define the rate random walk update equations
% These states integrate white noise (pure integrator model)
darwNew = [dax_rw; day_rw; daz_rw];

% Accel bias remains constant
dvbNew = dvz_b;
...
% Modified to include rate random walk states
processEqns = [qNew;vNew;pNew;dabNew;darwNew;dvbNew;vwnNew;vweNew;magNnew;magEnew;magDnew;magXnew;magYnew;magZnew];
...
% Modified to include rate random walk noise inputs
distVector = [da;dv];

% Define process noise vector for rate random walk states
% These are white noise inputs that drive the rate random walk states
syms dax_rw_noise day_rw_noise daz_rw_noise real
rwNoiseVector = [dax_rw_noise; day_rw_noise; daz_rw_noise];

% Combine all noise inputs
allNoiseVector = [distVector; rwNoiseVector];

% derive the control(disturbance) influence matrix
G = jacobian(processEqns, allNoiseVector);
[G,SG]=OptimiseAlgebra(G,'SG');

% derive the state error matrix
% Modified to include rate random walk process noise
imuNoise = diag([daxCov dayCov dazCov dvxCov dvyCov dvzCov dax_rwCov day_rwCov daz_rwCov]);
...
% sign functions to symbolic derivation which generates output with dirac
% functions
...
nStates = 25;  % Changed from 22 to 25
